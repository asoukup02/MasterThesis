...
The Kalman filter (KF), introduced by Kalman in 1960 \cite{Kalman1960KF}, is one of the most widely used algorithms for state estimation in dynamic systems. Its objective is to estimate the state of a system from noisy or incomplete sensor measurements by recursively combining a potentially uncertain mathematical model of the system with incoming measurements. At each time step, the filter first predicts the current state based on the system dynamics and then refines this prediction using the latest measurement. The prediction and measurement are weighted according to their respective uncertainties, allowing the filter to balance the information provided by the system model and the sensor measurements.

Under the assumptions of linear system dynamics and Gaussian (normally distributed) process and measurement noise, the Kalman filter yields the optimal state estimate in the minimum mean-square error (MMSE) sense. More specifically, at each time step $k$, it computes the state estimate that minimizes the expected squared estimation error conditioned on all available measurements,
...
This optimality property, combined with its capability to handle stochastic process and measurement models, makes the Kalman filter well suited for the estimation problem considered in this thesis. Atmospheric turbulence, as captured by the Dryden model (Section~\todo{ref}), is described as a stochastic process driven by white noise. Representing both the wind disturbance and the sensor uncertainty as random processes allows the filter to fuse the airship model with the available measurements in a statistically optimal and computationally efficient manner, updating the estimate recursively rather than reprocessing the full measurement history at every step.

 Atmospheric turbulence, as captured by the Dryden model (Section~\todo{ref}), is described as a stochastic process driven by white noise. By representing the wind disturbance and measurement uncertainties as random processes, the Kalman filter can recursively combine the airship dynamics model with the available sensor measurements to obtain an optimal state estimate under the assumed linear-Gaussian conditions. Furthermore, the recursive formulation enables the estimate to be updated using only the previous estimate and the latest measurement, eliminating the need to reprocess the complete measurement history at every time step.

The Kalman filter is formulated within a probabilistic state-space framework, where the system dynamics and measurement process are represented by stochastic linear equations. These equations are introduced in the following section.

The Kalman filter is formulated within a discrete probabilistic state-space framework, in which both the system dynamics and the measurement process are modeled as stochastic processes, state and output at any given time ar just random vectors according to a probability distribution \todo{define stochastic process}. 
This makes the Kalman Filter the perfect method given the stochastic description of the turbulence. We will use this white noise uncertainties to represent our wind model and to express model uncertainties.
The evolution of the system state is described by the discrete-time state equation, while the measurement equation relates the current state to the available sensor observations. To account for modeling inaccuracies and measurement uncertainties, as well as the stochastic wind, the process and measurement models are augmented by additive random variables/ vectors (\todo{define}) representing process noise and measurement noise, respectively.

The Kalman Filter generally consider normal (Gaussian) distributed random variables. A Gaussian random variable is classified by two quantities. Its mean and covariance. The mean equals the expected value of the variable, and the covariance of the variable can be interpreted as the uncertainty or the freedom of it, allowing for the variable to take on values that are further away from the mean more frequently. Figure~ illustrates two normal distributed variables. 

\todo{include Figure}

To understand how the KF works we consider the discrete probabilistic state-space model

\begin{align}
	\bm{x}_k &= \bm{A}_{k-1} \bm{x}_{k-1} + \bm{B}_{k-1} \bm{u}_{k-1} + \bm{q}_{k-1}, \\ 
	\vspace{5cm}
	\bm{y}_k &= \bm{H}_k \bm{x}_k + \bm{r}_k,
	\label{eq:discrete_system}
\end{align}

\todo{add dimensions}
where $\bm{x}_k$ denotes the system state, $\bm{y}_k$ the measurement vector, $u$ ..., $\bm{A}_{k-1}$ the state transition matrix, which may be time varying. $B$... and $\bm{H}_k$ the measurement matrix. The random vectors $\bm{q}_{k-1}$ and $\bm{r}_k$ represent the process and measurement noise, respectively, and are typically assumed to be zero-mean Gaussian white noise processes with covariance matrices $\bm{Q}_{k-1}$ and $\bm{R}_k$. Properties for Gaussian white noise processes, zero-mean white process noise with covariance $Q$, i.e.\ $E[w_k] = 0$ and $E[w_k w_k^T] = Q$. The process noise is assumed uncorrelated with the state, i.e.\ $E[(x_k - \bar{x}_k) w_k^T] = 0$. 

This leads to the result that now x and y are now random vectors and not deterministic anymore. The initial condition is not given by a deterministic value but represented by a prior distribution $\bm{x}_0 ~ N(m0, P0)$ where m0 and P0 are given. 

\begin{equation}
	x_0 \sim \mathcal{N}(\bar x_0, P_0).
\end{equation}

Depending on the uncertainty of both the state expressed by Pk and the measurement expressed by ?? the KF decides what the optimal value for the state estimation is.


The equation for the next estimate is given by: This is a very common structure for state estimation and can also be found in simple state estimator like the luenberger observer. \todo{cite}

\begin{equation}
	\hat{x}_{k}
	=
	\underbrace{A\hat{x}_{k-1}+Bu_{k-1}}_{\text{Prediction}}
	+
	\underbrace{K_k\left(y_k-\hat{y}_k\right)}_{\text{Update}},
	\qquad
	\hat{y}_k=C\left(A\hat{x}_{k-1}+Bu_{k-1}\right).
\end{equation}


The Kalman Filter works in two steps: A Prediction step, where using the model of the system the predicted next state, called the a priori estimate, as it only includes "old data", also the corresponding output is calculated and a update step where the output measurement is considered to find the estimated state.

\subsubsection{Prediction}

Since Eq.~\eqref{eq:discrete_system} is a linear operation of a Gaussian random variable, the state remains Gaussian at every subsequent time step,

\begin{equation}
	x_{k+1} \sim \mathcal{N}(\bar x_{k+1}, P_{k+1}),
\end{equation}

so that $x_{k+1}$ is fully characterized by its propagated mean $\bar x_{k+1}$ and covariance $P_{k+1}$.

By taking the expectation of Eq.~\eqref{eq:linear_system} and using $E[w_k] = 0$ the propagated mean computes to

\begin{equation}
	\bar x_{k+1} = E[x_{k+1}] = A \bar x_k + B u_k.
	\label{eq:mean_prop}
\end{equation}

The covariance at time step $k+1$ is defined as
\begin{equation}
	P_{k+1} = E\big[(x_{k+1} - \bar x_{k+1})(x_{k+1} - \bar x_{k+1})^T\big].
\end{equation}

Subtracting Eq.~\eqref{eq:mean_prop} from Eq.~\eqref{eq:linear_system}, the known input term $B u_k$ cancels, leaving the propagated estimation error

\begin{equation}
	x_{k+1} - \bar x_{k+1} = A(x_k - \bar x_k) + w_k.
	\label{eq:error_prop}
\end{equation}

Substituting Eq.~\eqref{eq:error_prop} into the covariance definition yields

\begin{align}
	P_{k+1} &= E\Big[\big(A(x_k - \bar x_k) + w_k\big)\big(A(x_k - \bar x_k) + w_k\big)^T\Big] \notag\\[2pt]
	&= \underbrace{E\big[A(x_k - \bar x_k)(x_k - \bar x_k)^T A^T\big]}_{A P_k A^T}
	+ \underbrace{E\big[w_k w_k^T\big]}_{Q}
	+ 2\underbrace{E\big[A(x_k - \bar x_k) w_k^T\big]}_{=\,0}.
	\label{eq:cov_expand}
\end{align}

The cross term vanishes because the process noise $w_k$ is by assumption uncorrelated with the state $x_k$. Since $A$ is deterministic, it may be pulled outside the expectation in the remaining terms, so that Eq.~\eqref{eq:cov_expand} reduces to the covariance propagation equation

\begin{equation}
	\boxed{P_{k+1} = A P_k A^T + Q.}
	\label{eq:cov_prop}
\end{equation}

Equations~\eqref{eq:mean_prop} and \eqref{eq:cov_prop} together describe how the first two moments of the state distribution evolve under the linear system model and constitute the prediction step of the Kalman filter \cite{Simon2006}.

Additionally the corresponding output is predicted.

This concludes the Prediction step.


\subsubsection{Update step}
Second the state measurement is used to update the prediction.

Therefore we consider the difference in the predicted system output and the measurement, which is called the innovation.

Compared to other state estimation methods
...
The presented equations showed the KF for linear systems. As the airship dynamics are nonlinear we need to 
...
\subsubsection{Probabilistic State-Space Formulation}

The Kalman filter operates on a discrete-time probabilistic state-space model, in which the system dynamics and the measurement process are described as stochastic processes rather than deterministic mappings. A \emph{random vector} is a vector-valued quantity whose value is not fixed but governed by a probability distribution over its possible outcomes; a \emph{stochastic process} is simply a sequence of random vectors indexed by time. Within this framework, the state $\bm{x}_k$ and the measurement $\bm{y}_k$ at any time step $k$ are treated as random vectors rather than deterministic quantities, and the filter tracks how their distributions evolve over time.

The Kalman filter further restricts these distributions to be Gaussian (normally distributed). A Gaussian random vector is fully characterized by two quantities: its mean, equal to the expected value of the vector, and its covariance, which quantifies the uncertainty about that mean -- a larger covariance means the vector is more likely to take values further from the mean. Figure~\todo{ref} illustrates two Gaussian random variables with different mean and covariance.

The evolution of the system is described by the discrete-time state equation, and the available measurements by the measurement equation,
\begin{align}
	\bm{x}_k &= A_{k-1} \bm{x}_{k-1} + B_{k-1} \bm{u}_{k-1} + \bm{q}_{k-1}, \label{eq:state_eq}\\
	\bm{y}_k &= H_k \bm{x}_k + \bm{r}_k, \label{eq:meas_eq}
\end{align}
where $\bm{x}_k \in \mathbb{R}^n$ is the system state, $\bm{y}_k \in \mathbb{R}^m$ the measurement, $\bm{u}_k \in \mathbb{R}^p$ the known input, $A_{k-1}$ the (possibly time-varying) state transition matrix, $B_{k-1}$ the input matrix, and $H_k$ the measurement matrix \todo{add dimensions / link to specific system}. The random vectors $\bm{q}_{k-1}$ and $\bm{r}_k$ represent the process and measurement noise, respectively. Both are modeled as zero-mean, white Gaussian noise processes with covariance matrices $Q_{k-1}$ and $R_k$,
\begin{equation}
	E[\bm{q}_{k}] = 0, \quad E[\bm{q}_{k}\bm{q}_{k}^T] = Q_k, \quad E[\bm{r}_k] = 0, \quad E[\bm{r}_k \bm{r}_k^T] = R_k,
\end{equation}
and the process noise is assumed uncorrelated with the state, i.e., $E[(\bm{x}_k - \bar{\bm{x}}_k)\bm{q}_k^T] = 0$.

As a consequence of this formulation, the state and the measurement are themselves random vectors rather than deterministic quantities. Likewise, the initial condition is not a fixed value but a prior distribution,
\begin{equation}
	\bm{x}_0 \sim \mathcal{N}(\bar{\bm{x}}_0, P_0),
\end{equation}
with prior mean $\bar{\bm{x}}_0$ and prior covariance $P_0$ assumed given.

Given this probabilistic description, the Kalman filter computes the updated state estimate as a weighted combination of the model-based prediction and the incoming measurement, where the weighting is determined by their relative uncertainties: the state covariance $P_k$ on one hand, and the measurement noise covariance $R_k$ on the other. This predictor-corrector structure,
\begin{equation}
	\hat{\bm{x}}_{k} = \underbrace{A_{k-1}\hat{\bm{x}}_{k-1} + B_{k-1}\bm{u}_{k-1}}_{\text{Prediction}} + \underbrace{K_k\left(\bm{y}_k - \hat{\bm{y}}_k\right)}_{\text{Update}}, \qquad \hat{\bm{y}}_k = H_k\left(A_{k-1}\hat{\bm{x}}_{k-1} + B_{k-1}\bm{u}_{k-1}\right),
\end{equation}
is common to many state estimators, including the simpler deterministic Luenberger observer \todo{cite}. The Kalman filter evaluates this structure in two steps: a \emph{prediction} step, which propagates the state and its uncertainty forward using the system model alone (the \emph{a priori} estimate), and an \emph{update} step, which corrects this prediction using the measurement residual $\bm{y}_k - \hat{\bm{y}}_k$ and the optimal gain $K_k$.

\subsubsection{Prediction}

Since Eq.~\eqref{eq:state_eq} is a linear operation on a Gaussian random vector, the state remains Gaussian at every subsequent time step,
\begin{equation}
	\bm{x}_{k+1} \sim \mathcal{N}(\bar{\bm{x}}_{k+1}, P_{k+1}),
\end{equation}
so that $\bm{x}_{k+1}$ is fully characterized by its propagated mean $\bar{\bm{x}}_{k+1}$ and covariance $P_{k+1}$.

Applying Eq.~\eqref{eq:state_eq} one time step ahead, $\bm{x}_{k+1} = A_k \bm{x}_k + B_k \bm{u}_k + \bm{q}_k$, and taking the expectation using $E[\bm{q}_k] = 0$, the propagated mean is
\begin{equation}
	\bar{\bm{x}}_{k+1} = E[\bm{x}_{k+1}] = A_k \bar{\bm{x}}_k + B_k \bm{u}_k.
	\label{eq:mean_prop}
\end{equation}

The covariance at time step $k+1$ is defined as
\begin{equation}
	P_{k+1} = E\big[(\bm{x}_{k+1} - \bar{\bm{x}}_{k+1})(\bm{x}_{k+1} - \bar{\bm{x}}_{k+1})^T\big].
\end{equation}

Subtracting Eq.~\eqref{eq:mean_prop} from the state equation above, the known input term $B_k \bm{u}_k$ cancels, leaving the propagated estimation error
\begin{equation}
	\bm{x}_{k+1} - \bar{\bm{x}}_{k+1} = A_k(\bm{x}_k - \bar{\bm{x}}_k) + \bm{q}_k.
	\label{eq:error_prop}
\end{equation}

Substituting Eq.~\eqref{eq:error_prop} into the covariance definition yields
\begin{align}
	P_{k+1} &= E\Big[\big(A_k(\bm{x}_k - \bar{\bm{x}}_k) + \bm{q}_k\big)\big(A_k(\bm{x}_k - \bar{\bm{x}}_k) + \bm{q}_k\big)^T\Big] \notag \\[2pt]
	&= \underbrace{E\big[A_k(\bm{x}_k - \bar{\bm{x}}_k)(\bm{x}_k - \bar{\bm{x}}_k)^T A_k^T\big]}_{A_k P_k A_k^T} + \underbrace{E\big[\bm{q}_k \bm{q}_k^T\big]}_{Q_k} + 2\underbrace{E\big[A_k(\bm{x}_k - \bar{\bm{x}}_k)\bm{q}_k^T\big]}_{=\,0}.
	\label{eq:cov_expand}
\end{align}
The cross term vanishes because the process noise $\bm{q}_k$ is by assumption uncorrelated with the state $\bm{x}_k$. Since $A_k$ is deterministic, it may be pulled outside the expectation in the remaining terms, so that Eq.~\eqref{eq:cov_expand} reduces to the covariance propagation equation
\begin{equation}
	\boxed{P_{k+1} = A_k P_k A_k^T + Q_k.}
	\label{eq:cov_prop}
\end{equation}

Equations~\eqref{eq:mean_prop} and \eqref{eq:cov_prop} together describe how the first two moments of the state distribution evolve under the linear system model and constitute the prediction step of the Kalman filter \cite{Simon2006}. The corresponding predicted output, $\hat{\bm{y}}_{k+1} = H_{k+1}\bar{\bm{x}}_{k+1}$, follows directly from the measurement equation.
